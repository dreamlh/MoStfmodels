...
flags.DEFINE_integer("lower_rank", None,
                    "Lower rank of softmax weights matrix.")
...
flags.DEFINE_bool("test_when_training", True,
                    "If true, test model after each epoch."
                    "If false, test model after the last epoch.")
  ...
  def __init__(self, is_training, config, input_):
    ...
    if n_experts == 0:
      softmax_w1 = tf.get_variable(
          "softmax_w1", [size, config.lower_rank], dtype=data_type())
      softmax_w2 = tf.get_variable(
          "softmax_w2", [config.lower_rank, vocab_size], dtype=data_type())
      softmax_b = tf.get_variable("softmax_b", [vocab_size], dtype=data_type())
      logits = tf.nn.xw_plus_b(output, tf.matmul(softmax_w1, softmax_w2), softmax_b)
      ...
      logit_w1 = tf.get_variable("logit_w1", [size, config.lower_rank], dtype=data_type())
      logit_w2 = tf.get_variable("logit_w2", [config.lower_rank, vocab_size], dtype=data_type())
      logit_b = tf.get_variable("logit_b", [vocab_size], dtype=data_type())
      logit = tf.nn.xw_plus_b(latent, tf.matmul(logit_w1, logit_w2), logit_b)
...
class RandomConfig(object):
  ...
  init_scale = random.uniform(0., 0.1)
  use_adam = False
  learning_rate = 1.0
  max_grad_norm = random.uniform(5,10)
  num_layers = random.randint(1,3)
  max_epoch = random.randint(5,15)
  max_max_epoch = max_epoch + 40
  keep_prob = random.uniform(0.3, 0.5)
  lr_decay = random.uniform(0.75,1.)
  vocab_size = random.randrange(10000,15001,10000)
  n_experts = random.randint(2,20)

  while True:
      num_steps = random.randrange(20,51,5)
      batch_size = random.randrange(10,31,2)
      hidden_size = random.randrange(500,1501,100)
      if num_steps*batch_size*hidden_size<=1200000:
          break

  rnn_mode_choice = random.randint(0,2)
  if rnn_mode_choice == 0:
      rnn_mode = BASIC
  elif rnn_mode_choice == 1:
      rnn_mode = BLOCK
  else:
      rnn_mode = CUDNN

  use_dynamic = False if random.randint(0,1) else True
  lower_rank = hidden_size // 4
...
def get_config():
  ...
  if FLAGS.lower_rank is not None:
      config.lower_rank = FLAG.lower_rank
  if FLAGS.use_dynamic is not None:
      config.use_dynamic = FLAGS.use_dynamic
  if FLAGS.init_scale is not None:
      config.init_scale = FLAGS.init_scale
  if FLAGS.use_adam is not None:
      config.use_adam = FLAGS.use_adam
...
def print_config(config):
  ...
  print('lower_rank={}'.format(config.lower_rank))
...
def main(_):
  ...
  print("Gragh construction time: %.3f" % (time.time() - global_begin_time), flush=True)
  ...
  with tf.Graph().as_default():
    ...
    with sv.managed_session(config=config_proto) as session:
      for i in range(config.max_max_epoch):
        ...
        print("Epoch: %d Learning rate: %.8f" % (i + 1, session.run(m.lr)), flush=True)
        ...
        if FLAGS.test_when_training:
            ...
            print("Epoch: %d Test Perplexity: %.3f" % (i + 1, test_perplexity), flush=True)

        print("Current running time: %.3f" % ((time.time() - global_begin_time)/3600), flush=True)
      ...
      if config.rnn_mode != CUDNN:
        print("The number of parameters: {:.3f}".format(get_num_params()/1000000), flush=True)

      if FLAGS.num_gpus:
        print("Peak memory usage of GPUs: {}".format(session.run(m.memory_use)/(1024**3)), flush=True)
  ...
  print("Global duration time: %.3f" % ((time.time() - global_begin_time)/3600), flush=True)
