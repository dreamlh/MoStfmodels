...
flags.DEFINE_bool("use_adam", None,
...
flags.DEFINE_bool("use_dynamic", None,
...
class PTBModel(object):
  ...
  def __init__(self, is_training, config, input_):
    ...
    if n_experts == 0:
      ...
      logits = tf.matmul(output, tf.transpose(embedding, [1,0]))
      ...
      logit = tf.matmul(latent, tf.transpose(embedding, [1,0]))
...
def run_epoch(session, model, eval_op=None, parallel=False, verbose=False):
  """Runs the model on the given data."""
  start_time = time.time()
  costs = 0.0
  iters = 0
  state = session.run(model.initial_state)
  if parallel and FLAGS.num_gpus > 1:
    epoch_size = model.input.epoch_size // FLAGS.num_gpus
  else:
  ...
  with tf.Graph().as_default():
    ...
    with sv.managed_session(config=config_proto) as session:
      for i in range(config.max_max_epoch):
        ...
        train_perplexity = run_epoch(session, m, eval_op=m.train_op, parallel=True, verbose=True)
